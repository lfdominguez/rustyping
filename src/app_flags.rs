...
pub fn configure() -> Result<(), String> {
    let flags = App::new(option_env!("CARGO_PKG_NAME").unwrap_or("UNKNOWN"))
        ...
        .arg(Arg::with_name("ping_timeout")
            .short("t")
            .long("ping_timeout")
        ...
        .arg(Arg::with_name("ping_interval")
            .short("i")
            .long("ping_interval")
    ...
    redis_host_str.parse::<Ipv4Addr>().and_then(|ipv4| {
        is_v4 = true;
    ...
    if !is_v4 {
        redis_host_str.parse::<Ipv6Addr>().and_then(|ipv6| {
            is_v6 = true;
    ...
    let timeout = flags.value_of("ping_timeout").unwrap().parse::<f64>().unwrap_or(1.0);
    let interval = flags.value_of("ping_interval").unwrap().parse::<i64>().unwrap_or(5);
